(() => {
  ...
  const staffPortalUrl = window.LUMORA_STAFF_PORTAL_URL || '/login.html';
  const routes = {
  ...
  document.querySelectorAll('nav').forEach((nav) => {
    if (!nav.querySelector('.lumora-staff-portal-link')) {
      const link = document.createElement('a');
      link.className = 'lumora-staff-portal-link';
      link.href = staffPortalUrl;
      link.textContent = 'Staff Portal';
      link.style.cssText = 'display:inline-block;margin-left:1rem;color:#d4af37;font-size:.75rem;letter-spacing:.12em;text-transform:uppercase;';
